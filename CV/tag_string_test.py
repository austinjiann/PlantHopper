...
def build_cmd_line(tag_id: int, found: bool, dx_m: float, pitch_deg: float, shoot: bool):
    return (
        f"cmd:search;"
        f"id:{tag_id};"
        ...
        if ids is not None and len(ids) > 0:
            ...
            if camera_matrix is not None and dist_coeffs is not None:
                ...
                for i, (rvec, tvec) in enumerate(zip(rvecs, tvecs)):
                    ...
                    # Instead of sending serial, print the message
                    if tag_id == 1:
                        dx_m = float(tvec[0])
                        pitch_deg = float(rpy_deg[1])
                        msg = build_cmd_line(tag_id=1, found=True, dx_m=dx_m, pitch_deg=pitch_deg, shoot=False)
                        print(msg)

            else:
                cv2.putText(
                    frame,
                    "Provide --calib to compute pose (rpy/dx/dy).",
                    (20, 40),
